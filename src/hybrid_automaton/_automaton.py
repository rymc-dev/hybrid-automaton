...
class Automaton:
    ...
    args:
        ...
        version: str
            version string for this automaton's model definition
        states: List[State]
    ...
    async def activate(
        ...
        *runtime* initial conditions of the system, including the continuous state `x`,
        ...
        Notes:
            ...
            • `initial_continuous_state` represents the continuous state that will be
            updated over time through the state's flow() function and integrated using
            the provided integration method (unless in real-time mode).

            • `initial_auxiliary_states` represent auxiliary continuous variables that
            do not undergo automatic integration, but may be accessed by:
                – guard functions
                – invariant checks
                – reset maps
                – flow functions
            ...
            • `initial_control_input_states` contain static or piecewise-static control
            inputs that may be referenced by flow functions or guards. These are not
            integrated and are only updated externally via `set_control_input()`.

        Args:
            initial_continuous_state (Optional[ContinuousState]):
            ...
            initial_control_input_states (Optional[List[ControlState]]):
                Initial control inputs. These remain static unless updated by
                `set_control_input()`.

            delta_time (float):
                Expected delta time between evaluation steps of the automaton,
                defaults to `0.01`.

        Example:
            >>> ha = Automaton(name="vessel_controller", version="1.0.0", states=[q1, q2])
            >>> results = await ha.activate(
            ...     initial_continuous_state=ContinuousState(
            ...         "vessel_state",
            ...         x0=np.array([np.deg2rad(100), 5.0]),
            ...         x_labels=['heading', 'speed'],
            ...     ),
            ...     initial_control_input_states=[
            ...         ControlState("rudder_offset", u0=np.array([-2.0])),
            ...     ],
            ...     delta_time=1.0,
            ... )
        """
